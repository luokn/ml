...
class DecisionTree:
    ...
    def _create_tree(self, indices, features):
        ...
        for val in np.unique(self.X[indices, best_feature]):  # 为该特征的每一个取值都建立子树
    ...
    def _query_indices(self, indices, feature, value):
        return indices[self.X[indices, feature] == value]
    ...
    def _calc_entropy(self, indices):  # 计算经验熵
        prob = np.bincount(self.Y[indices]) / len(indices)  # 采用二进制计数法，x必须为正整数向量
        prob = prob[prob != 0]  # 除去0概率
        return np.sum(prob * -np.log(prob))  # 经验熵

    def _calc_cond_entropy(self, indices, feature):  # 计算条件熵
        cond_ent = 0  # 经验条件熵
        for val in np.unique(self.X[indices, feature]):
    ...
    def _select_class(self, indices):
        prob = np.bincount(self.Y[indices]) / len(indices)  # 计算类别频率
        c = np.argmax(prob)
